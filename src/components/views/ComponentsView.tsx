...
export const ComponentsView: React.FC = () => {
  ...
  const tagComp = (t: string, cls: any) => (
    <Tag key={t} variant={cls} style={{ ...mono, fontSize: '10.5px', padding: '1px 6px' }}>
      {t}
    </Tag>
  );

  const btnComp = (t: string, variant: any, style?: React.CSSProperties) => (
  ...
  const specCards = [
    ...
    },
    {
      ...
      demo: (
        <div style={{ borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', boxShadow: 'var(--shadow-md)', padding: '5px', width: '220px' }}>
          <div style={{ padding: '4px 9px 6px', ...mono, fontSize: '11px', color: 'var(--fg3)', borderBottom: '1px solid var(--line)', marginBottom: '4px' }}>
            feature/mlx5-next
          </div>
          <div style={{ padding: '3px 9px', fontSize: '12.5px', borderRadius: '4px', background: 'var(--sel)' }}>Checkout</div>
          <div style={{ padding: '3px 9px', fontSize: '12.5px' }}>Merge into main</div>
          <div style={{ height: '1px', background: 'var(--line)', margin: '4px 6px' }} />
          <div style={{ padding: '3px 9px', fontSize: '12.5px', color: 'var(--del)' }}>Delete branch</div>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      demo: (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', height: '26px', padding: '0 8px', background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: '5px', fontSize: '11.5px', ...mono, color: 'var(--fg2)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', height: '26px', padding: '0 8px', background: 'var(--raised)', border: '1px solid var(--line)', borderRadius: '5px', fontSize: '11px' }}>
            <Tag variant="outline" style={{ fontSize: '10px', padding: '0 6px', color: 'var(--warn)', borderColor: 'var(--warn)' }}>
              REBASING
            </Tag>
            Step 2 of 5
            <span style={{ flex: 1 }} />
            {btnComp('Continue', 'primary', { height: '18px', fontSize: '10.5px' })}
            {btnComp('Abort', 'primary', { height: '18px', fontSize: '10.5px', color: 'var(--del)', borderColor: 'var(--del)' })}
          </div>
        </>
      )
    },
      ...
      demo: (
        <Card elevation="md">
          <div style={{ display: 'flex', gap: '8px' }}>
            <span style={{ flex: 1, fontSize: '12.5px', fontWeight: 500 }}>Updating 7 submodules</span>
            {btnComp('Cancel', 'secondary', { height: '19px', fontSize: '10.5px' })}
          </div>
          <div style={{ ...mono, fontSize: '11px', color: 'var(--fg3)' }}>tools/lib/bpf — receiving objects 42%</div>
          <div style={{ height: '3px', borderRadius: '2px', background: 'var(--raised2)' }}>
            <div style={{ height: '100%', width: '42%', background: 'var(--color-accent)', borderRadius: '2px' }} />
          </div>
        </Card>
      )
    },
    {
      ...
      demo: (
        <div className="dialog elev-lg" style={{ width: '100%', padding: 'var(--space-3)' }}>
          <div className="dialog-title" style={{ fontSize: '15px' }}>Hard reset main to origin/main?</div>
          <div className="dialog-body" style={{ fontSize: '12px' }}>12 modified files will be permanently discarded.</div>
          <div style={{ padding: '6px 9px', border: '1px solid var(--line)', borderRadius: '6px', background: 'var(--color-bg)', ...mono, fontSize: '11px', color: 'var(--fg2)' }}>
    ...
    },
    {
      ...
      demo: (
        <div style={{ ...mono, fontSize: '11.5px', border: '1px solid var(--line)', borderRadius: '6px', overflow: 'hidden' }}>
          <div style={{ background: 'var(--raised)', color: 'var(--fg3)', padding: '2px 8px' }}>@@ -412,9 +412,14 @@</div>
          <div style={{ background: 'var(--delbg)', color: 'var(--del)', padding: '2px 8px' }}>− if (unlikely(!priv-&gt;channels.num))</div>
          <div style={{ background: 'var(--addbg)', color: 'var(--add)', padding: '2px 8px' }}>+ if (unlikely(!priv-&gt;channels.num ||</div>
        </div>
      )
    },
    {
      ...
      demo: (
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <svg width={70} height={30} style={{ display: 'block', flex: '0 0 auto' }}>
            <path d="M12 0L12 15" stroke={COLORS[0]} strokeWidth={1.7} fill="none" strokeLinecap="round" />
            <path d="M12 15L12 30" stroke={COLORS[0]} strokeWidth={1.7} fill="none" strokeLinecap="round" />
            <path d="M12 15C12 22.5,27 22.5,27 30" stroke={COLORS[1]} strokeWidth={1.7} fill="none" strokeLinecap="round" />
            <path d="M42 0L42 30" stroke={COLORS[2]} strokeWidth={1.7} fill="none" strokeLinecap="round" />
            <circle cx={12} cy={15} r={4.4} fill="var(--color-bg)" stroke={COLORS[0]} strokeWidth={1.9} />
          </svg>
          <span style={{ fontSize: '11.5px', color: 'var(--fg3)' }}>merge commit, lane 0 → lanes 0 + 1</span>
        </div>
      )
    }
  ];
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', minHeight: 0, padding: 'var(--space-8) var(--space-8) 70px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(330px,1fr))', gap: 'var(--space-4)' }}>
        {specCards.map((s, i) => (
          <Card key={i} elevation="sm" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: 'var(--space-3)', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'baseline', gap: 'var(--space-2)' }}>
              <span style={{ fontSize: '12.5px', fontWeight: 500 }}>{s.title}</span>
              <span style={{ fontSize: '11px', color: 'var(--fg3)', fontFamily: 'var(--font-mono)' }}>{s.tag}</span>
            </div>
            <div style={{ padding: 'var(--space-4) var(--space-3)', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 'var(--space-2)', background: 'var(--color-bg)' }}>
              {s.demo}
            </div>
            <div style={{ padding: 'var(--space-2) var(--space-3)', borderTop: '1px solid var(--line)', fontSize: '11px', color: 'var(--fg3)', textWrap: 'pretty' }}>
              {s.note}
            </div>
          </Card>
        ))}
