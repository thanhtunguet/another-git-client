...
export const CommitDetailsView: React.FC = () => {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
      <div style={{ flex: '0 0 380px', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--line)', background: 'var(--panel)', minHeight: 0 }}>
        <div style={{ flex: '0 0 auto', padding: 'var(--space-4)', borderBottom: '1px solid var(--line)' }}>
          <h6 style={{ margin: '0 0 6px', color: 'var(--color-accent)' }}>{detailKicker}</h6>
          <div style={{ fontSize: '14px', fontWeight: 500, marginBottom: 'var(--space-2)', textWrap: 'pretty' }}>
            {detailSubject}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', fontSize: '11.5px', color: 'var(--fg2)' }}>
            <span>{detailAuthor}</span>
            <span style={{ color: 'var(--fg3)', fontFamily: 'var(--font-mono)' }}>{detailDate}</span>
            <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--iris)' }}>{detailHash}</span>
          </div>
        </div>

        <div style={{ flex: '0 0 auto', display: 'flex', gap: '6px', padding: 'var(--space-2) var(--space-3)', borderBottom: '1px solid var(--line)', flexWrap: 'wrap' }}>
          <Button variant="secondary" style={{ height: '23px', fontSize: '11.5px' }} onClick={() => setView('diff')}>
            {`Open diffs (${dfiles.length})`}
          </Button>
          <Button variant="secondary" style={{ height: '23px', fontSize: '11.5px' }} onClick={act('Revert selected changes')}>
            Revert selected
          </Button>
          <Button variant="secondary" style={{ height: '23px', fontSize: '11.5px' }} onClick={act('Cherry-pick selected changes')}>
            Cherry-pick selected
          </Button>
          <Button variant="secondary" style={{ height: '23px', fontSize: '11.5px' }} onClick={act('Create patch')}>
            Create patch…
          </Button>
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 'var(--space-2) 0', minHeight: 0 }}>
          {Object.keys(dirs).map(d => (
            <React.Fragment key={d}>
              ...
              >
                <i className="ph ph-caret-down" style={{ fontSize: '11px', color: 'var(--fg3)', width: '10px' }} />
                <span>{d}/</span>
              </div>
              {dirs[d].map((f, k) => (
                <div
                ...
                >
                  <span style={{ width: '11px', textAlign: 'center', color: statusColor(f.status), fontWeight: 600 }}>
                    {f.status}
                  </span>
