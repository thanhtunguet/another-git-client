...
- Clear All writes `clear` on Linux/macOS and `cls` on Windows only to the terminal pane that opened the context menu.
...
- Prevented React Strict Mode's development-only effect replay from allowing a stale cleanup to stop the newly-created terminal session.
